...
if(!empty($_GET['inserir'])){
    $_SESSION['carr'] = $_GET['inserir'];
    $carr = $_SESSION['carr'];
}
if(!empty($_GET['id_produto'])){
    $id_p = $_GET['id_produto'];
    $sql ='UPDATE tela_principal SET id_car_prod_'.$carr.'=? WHERE id_tela = 1' ;
    try {
    ...
    header('location:troca_carr.php?mensagem='.$msg);
}

if(!empty($_GET['excluir_carr'])){
    $carr = $_GET['excluir_carr'];
    $id_p = '';
    $sql ='UPDATE tela_principal SET id_car_prod_'.$carr.'=? WHERE id_tela = 1' ;
    try {
    $insercao = $conexao->prepare($sql);
...
<html lang="pt-br">
 ...
 <div class="card mt-2" role="document">
                    <div class="card-header">
                        <h3 class="text-info">Opções de Multicards</h3>
                    </div>
     <div class="card-body">
         <?php
            if(!empty($carr)){
            $id = $dados1['id_car_prod_'.$carr];
            if(!empty($id)){
  ...
  <div class="modal-dialog">
    <div class="modal-content ">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Escolha seu Card novo...</h5>
