  ...
  <div class="container">
	...
	<div class="card mt-2">
            <div class="card-header">
                <h3 class="text-info">Pesquisa de compras por valores</h3>
            </div>
            <div class="card-body">
                <h3>Escolha uma das opções para a busca desejada</h3>
				<a class="btn btn-warning" href="?mais_valor=ok">Maiores valores</a>
				<a class="btn btn-warning" href="?menos_valor=ok">Menores valores</a>
				
				<p><strong>Insira o intervalo de valores para a pesquisa:</strong></p>
                ...
                <div class="row">
                <div class="col-sm-4">
                <input type="search" class="form-control" name="valor_inicio"/>
				</div>
				<div class="col-sm-4">
				<input type="search" class="form-control" name="valor_final"/>
				</div>
				<div class="col-sm-2">
		...
		<?php
if(!empty($_GET['valor_inicio']) || !empty($_GET['valor_final'])){
				if(!empty($_GET['valor_inicio'])){ 
					$valor1 = $_GET['valor_inicio'];}else{ $valor1 = 0;}
				if(!empty($_GET['valor_final'])){ 
					$valor2 = $_GET['valor_final'];}else{ $valor2 = 50000;}
					
				if(is_numeric($valor1) && is_numeric($valor2)){							
				$sql_2 = "SELECT * FROM compras WHERE total BETWEEN '".$valor1."' AND '".$valor2."'";
					  ...
					  <div class="modal-dialog" >
						<div class="modal-content ">
						  <div class="modal-header bg-info">
							<h3 class="modal-title">Compras efetuadas com valor de "R$ '.number_format($valor1,2,',','.').'" até "R$ '.number_format($valor2,2,',','.').'".</h3>
						   </div>
...
//Comecarei aki a paginação
if(!empty($_GET['menos_valor'])){
$quantidade_cp = 15; 
$pagina     = (isset($_GET['pagina'])) ? (int)$_GET['pagina'] : 1;
$inicio     = ($quantidade_cp * $pagina) - $quantidade_cp;
    $sql_a = "SELECT * FROM compras ORDER BY total ASC LIMIT ".$inicio.",".$quantidade_cp."";
	...
	echo '<div class="modal fade modal-xl"  id="exemplomodal">
					  <div class="modal-dialog" >
						<div class="modal-content ">
						  <div class="modal-header bg-info">
							<h3 class="modal-title">Compras efetuadas com os menores valores</h3>
						   </div>
						  <div class="modal-body bg-light">';
								echo '<table  align="center" border="3" class="border-secondary" style="table-layout: fixed;width:1113px">';
								echo '<thead style="display: block;position: relative;" class="border">';
...
$sqlTotal = "SELECT COUNT(id_compra) AS id FROM compras";
  //Executa o SQL
...
$qrTotal = $consulta_c->fetch(PDO::FETCH_ASSOC);
  ...
  /**
   ...
   echo '<div id="navegacao" align="center">';
        ...
        echo '<a  class="btn btn-primary mb-2" href="?menos_valor=ok&pagina=1">primeira</a> | ';
        echo "<a  class='btn btn-primary mb-2' href=\"?menos_valor=ok&pagina=$anterior\">anterior</a> | ";
   ...
   for($i = $pagina-$exibir; $i <= $pagina-1; $i++){
       if($i > 0)
        echo '<a  class="btn btn-primary mb-2 " href="?menos_valor=ok&pagina='.$i.'"> '.$i.' </a>';
  }

  echo '<a  class="btn btn-primary mb-2 ms-1" href="?menos_valor=ok&pagina='.$pagina.'"><strong>'.$pagina.'</strong></a>';

  for($i = $pagina+1; $i < $pagina+$exibir; $i++){
       if($i <= $totalPagina)
        echo '<a class="btn btn-primary mb-2 ms-1" href="?menos_valor=ok&pagina='.$i.'"> '.$i.' </a>';
  }
    ...
    */
     ...
     echo " | <a class='btn btn-primary mb-2' href=\"?menos_valor=ok&pagina=$posterior\">próxima</a> | ";
    echo "  <a class='btn btn-primary mb-2' href=\"?menos_valor=ok&pagina=$totalPagina\">última</a>";
   
}
        
 if(!empty($_GET['mais_valor'])){
$quantidade_cp = 15; 
$pagina     = (isset($_GET['pagina'])) ? (int)$_GET['pagina'] : 1;
$inicio     = ($quantidade_cp * $pagina) - $quantidade_cp;
    $sql_a = "SELECT * FROM compras ORDER BY total DESC LIMIT ".$inicio.",".$quantidade_cp."";
	$consulta_a = $conexao->query($sql_a);
	$dados = $consulta_a->fetchALL(PDO::FETCH_ASSOC);
...
$qrTotal = $consulta_c->fetch(PDO::FETCH_ASSOC);
  ...
  /**
   ...
   echo '<div id="navegacao" align="center">';
        ...
        echo '<a  class="btn btn-primary mb-2" href="?mais_valor=ok&pagina=1">primeira</a> | ';
        echo "<a  class='btn btn-primary mb-2' href=\"?mais_valor=ok&pagina=$anterior\">anterior</a> | ";
   ...
   for($i = $pagina-$exibir; $i <= $pagina-1; $i++){
       if($i > 0)
        echo '<a  class="btn btn-primary mb-2 " href="?mais_valor=ok&pagina='.$i.'"> '.$i.' </a>';
  }

  echo '<a  class="btn btn-primary mb-2 ms-1" href="?mais_valor=ok&pagina='.$pagina.'"><strong>'.$pagina.'</strong></a>';

  for($i = $pagina+1; $i < $pagina+$exibir; $i++){
       if($i <= $totalPagina)
        echo '<a class="btn btn-primary mb-2 ms-1" href="?mais_valor=ok&pagina='.$i.'"> '.$i.' </a>';
  }
    ...
    */
     ...
     echo " | <a class='btn btn-primary mb-2' href=\"?mais_valor=ok&pagina=$posterior\">próxima</a> | ";
    echo "  <a class='btn btn-primary mb-2' href=\"?mais_valor=ok&pagina=$totalPagina\">última</a>";
